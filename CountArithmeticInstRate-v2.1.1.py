...
This version (v2.1.1) remove the limitation of output that limits the instruction larger than 8
...
def OutputToFile(OutputFile, bblResult, minRate):
    with open(OutputFile, 'w') as f1:
        ...
        for i in xrange(10):
            ...
            fmt = "BBL math rate range : {[%02d%%] <= rate < [%02d%%]} has [%05d] block.\n" % (i*10, (i+1)*10, res)
            f1.write(fmt)
        fmt = "BBL math rate range : {[%02d%%] <= rate < [%02d%%]} has [%05d] block.\n" % (100, 100, statisticResult[10])
        f1.write(fmt)
        fmt = "Total BBK has [%05d] block.\n" % (bblCnt)
        f1.write(fmt)
        f1.write("\n")
