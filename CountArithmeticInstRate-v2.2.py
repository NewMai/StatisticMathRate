...
def OutputToFile(OutputFile, bblResult, minRate, minInst):
    with open(OutputFile, 'w') as f1:
        ...
        fmt = "Statistic total basic block.:\n"
        f1.write(fmt)
        for i in xrange(10):
            res = statisticResult[i]
            fmt = "BBL math rate range : {[%02d%%] <= rate < [%02d%%]} has [%05d] blocks.\n" % (i*10, (i+1)*10, res)
            f1.write(fmt)
        fmt = "BBL math rate range : {[%02d%%] <= rate < [%02d%%]} has [%05d] blocks.\n" % (100, 100, statisticResult[10])
        f1.write(fmt)
        fmt = "Total BBK has [%05d] blocks.\n" % (bblCnt)
        f1.write(fmt)
        f1.write("\n")
...
def Main():
    ...
    try:
        ...
        OutputToFile(outputFileName, bblResult, minRate, minInst)
        print "Finish!"
